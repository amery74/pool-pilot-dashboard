class PoolPilotDashboardCard extends HTMLElement {
  ...
  _chlStatus() {
    const isChl = this._resolvedDisinfectionMode() === "chlorine";
    const raw = isChl ? this._num(this.config.chlorine_entity) : this._num(this.config.orp_entity);
    if (raw === null) return "unknown";
    const target = this._num(isChl ? this.config.target_fc_entity : this.config.target_orp_entity);
    if (target !== null) {
      const lowDelta = isChl ? Math.max(0.2, target * 0.2) : 50;
      const highDelta = isChl ? Math.max(0.4, target * 0.4) : 100;
      if (raw < target - lowDelta) return "low";
      if (raw > target + highDelta) return "high";
      return "good";
    }
    if (isChl) { if (raw < 1) return "low"; if (raw > 3) return "high"; return "good"; }
    if (["bromine", "active_oxygen"].includes(this._poolType())) return "good";
    if (raw < 650) return "low";
    if (raw > 800) return "high";
    return "good";
  }
  _norm(v) {
    return String(v ?? "")
  ...
  _alertIssues() {
    const issues = [], ph = this._phStatus(this._value(this.config.ph_entity, "")), dis = this._chlStatus();
    if (ph === "high") issues.push("corriger le pH trop haut");
    if (ph === "low") issues.push("corriger le pH trop bas");
    if (dis === "high") issues.push(this._disinfectionIssue("high"));
    if (dis === "low") issues.push(this._disinfectionIssue("low"));
    const actions = this.config.actions_entity ? String(this._value(this.config.actions_entity, "")).trim() : "";
    if (actions && !/^(ok|bon|good|normal|aucune|none|ras|rien|—|-|unknown|unavailable)$/i.test(actions) && /(ajouter|corriger|correction|dosage|produit|ph\+|ph-|chlore|brome|oxygene|oxygène|redox|electroly|électroly|traitement|boost)/i.test(actions)) issues.push(actions);
  ...
  _poolType() {
    const c = this.config || {};
    const ids = [c.action_summary_entity, c.actions_entity, c.alert_status_entity, c.pool_alerts_entity].filter(Boolean);
    for (const id of ids) {
      const p = this._state(id)?.attributes?.pool_type;
      if (p) return String(p);
    }
    const found = Object.values(this._hass?.states || {}).find((s) => s?.attributes?.pool_type);
    return String(found?.attributes?.pool_type || "chlorine");
  }
  _treatmentLabel() {
    return {chlorine:"Chlore",saltwater:"Sel / électrolyse",bromine:"Brome",active_oxygen:"Oxygène actif"}[this._poolType()] || "Chlore";
  }
  _treatmentIcon() {
    return {chlorine:"mdi:flask-outline",saltwater:"mdi:creation-outline",bromine:"mdi:water-opacity",active_oxygen:"mdi:water-plus-outline"}[this._poolType()] || "mdi:flask-outline";
  }
  _disinfectionIssue(direction) {
    const t = this._poolType();
    if (t === "saltwater") return direction === "low" ? "augmenter la production de l’électrolyseur" : "réduire la production de l’électrolyseur";
    if (t === "bromine") return direction === "low" ? "renforcer le traitement au brome" : "surveiller un niveau de désinfection trop élevé";
    if (t === "active_oxygen") return direction === "low" ? "renforcer le traitement à l’oxygène actif" : "suspendre temporairement l’apport d’oxygène actif";
    return direction === "low" ? "corriger le niveau de chlore trop bas" : "corriger le niveau de chlore trop haut";
  }
  _treatmentBoostAdvice() {
    const t = this._poolType();
    if (t === "saltwater") return "Augmentez temporairement la production de l’électrolyseur ou activez le mode Boost.";
    if (t === "bromine") return "Renforcez le traitement au brome selon le produit configuré dans le Pool House.";
    if (t === "active_oxygen") return "Ajoutez la dose d’oxygène actif recommandée par Pool Pilot.";
    return "Réalisez le traitement chloré recommandé par Pool Pilot.";
  }

  _resolvedDisinfectionMode() {
    if (["bromine", "active_oxygen"].includes(this._poolType())) return "orp";
    const c = this.config || {}, intel = this._chemistryIntel?.() || {}, s = this._state(c.disinfection_mode_entity);
    const raw = String(intel.disinfection_mode ?? intel.detail?.disinfection_mode ?? s?.state ?? s?.attributes?.disinfection_mode ?? c.disinfection_mode ?? "auto").toLowerCase();
    if (raw.includes("orp") || raw.includes("redox")) return "orp";
    if (raw.includes("chlor") || raw.includes("free")) return "chlorine";
    return c.chlorine_entity ? "chlorine" : "orp";
  }
  _chlorineGauge() {
    const isChl = this._resolvedDisinfectionMode() === "chlorine",
      ...
      label = isChl ? "Chlore libre" : "RedOx / ORP",
      display = raw === null ? "—" : isChl ? raw.toFixed(1).replace(".", ",") : Math.round(raw),
      unit = isChl ? "ppm" : "mV";
    return `<div class="gauge ph-gauge chlorine-gauge"><svg viewBox="0 0 140 140" class="ph-svg"><path class="ph-track" d="${this._arc(70,70,54,-135,135)}"/><path class="ph-progress" d="${this._arc(70,70,54,-135,end)}"/><circle class="ph-dot" cx="${p.x}" cy="${p.y}" r="10"/><text x="70" y="72" text-anchor="middle" class="disinfection-value">${display}</text><text x="70" y="90" text-anchor="middle" class="gauge-unit">${unit}</text></svg><div class="gauge-value gauge-value-placeholder" aria-hidden="true">&nbsp;</div><div class="gauge-label">${label}</div><div class="pill ${this._qualityClass(status)}">${this._statusIcon(status)}${this._labelState(status)}</div></div>`;
  }
  ...
  _coverEntity() {
    return this.config?.cover_entity || "";
  }
  _coverControl() {
    const e=this._coverEntity(), st=this._state(e), pos=st?.attributes?.current_position;
    return `<div class="control-device cover-control"><ha-icon icon="mdi:garage-variant"></ha-icon><div><strong>Volet</strong><span>${st?.state || "—"}${pos !== undefined ? ` · ${pos} %` : ""}</span></div><div class="cover-buttons"><button title="Ouvrir" data-cover-action="open" data-entity="${e}"><ha-icon icon="mdi:arrow-up"></ha-icon></button><button title="Arrêter" data-cover-action="stop" data-entity="${e}"><ha-icon icon="mdi:stop"></ha-icon></button><button title="Fermer" data-cover-action="close" data-entity="${e}"><ha-icon icon="mdi:arrow-down"></ha-icon></button></div></div>`;
  }
  _filterControl() {
    const c = this.config || {};
  ...
  _smartFiltrationInfo() {
    ...
    const pctRaw = a.filtration_progress_percent ?? detail.filtration_progress_percent;
    const pct = Number(pctRaw);
    const remaining = a.filtration_remaining_hours ?? detail.filtration_remaining_hours ?? "—";
  ...
  _expertPanel() {
    ...
    return `<div class="full-sheet expert-sheet"><div class="sheet-top"><button data-panel="menu"><ha-icon icon="mdi:arrow-left"></ha-icon></button><h2>Mode Expert</h2><button data-panel=""><ha-icon icon="mdi:close"></ha-icon></button></div><div class="expert-scroll"><div class="section-title">Données brutes</div><div class="raw-box"><table><thead><tr><th>Date</th><th>Src</th><th>pH</th><th>ORP</th><th>Temp</th></tr></thead><tbody>${rows}</tbody></table></div><div class="section-title">Test bandelette</div><div class="strip-card"><div class="strip-tabs single"><button class="active">Formulaire</button></div><div class="strip-form analyseur_eau-strip">${inp("strip_ph", v("ph", ""), "", "mdi:palette-outline", "pH", `<input name="strip_temperature" type="number" step="0.1" value="${v("temperature", "")}"><span>°C</span>`, "ph-row")}${inp("strip_alkalinity", v("alkalinity", ""), "ppm", "mdi:snowflake", "TAC", `<input name="strip_alkalinity_f" data-convert-from="strip_alkalinity" type="number" step="0.1" value="${tacF}"><span>°f</span>`, "tac-row")}${inp("strip_calcium", v("calcium", ""), "ppm", "mdi:ellipse", "TH", `<input name="strip_calcium_f" data-convert-from="strip_calcium" type="number" step="0.1" value="${thF}"><span>°f</span>`, "th-row")}${inp("strip_cya", v("cya", ""), "ppm", "mdi:link-variant", "Stabilisant")}${inp("strip_free_chlorine", v("free_chlorine", ""), "ppm", "mdi:triangle-outline", "Chlore libre")}${inp("strip_total_chlorine", v("total_chlorine", ""), "ppm", "mdi:triangle", "Chlore total")}<button class="save-strip" data-save-strip="1">Valider</button></div></div>${this._stripRangesPanel(strip)}<div class="section-title">Traitement</div><div class="diag smart-diag"><div>Traitement principal<span><ha-icon icon="${this._treatmentIcon()}"></ha-icon> ${this._treatmentLabel()}</span></div><div>Mesure désinfection<span>${this._resolvedDisinfectionMode()==="orp"?"RedOx / ORP":"Chlore libre"}</span></div></div><div class="section-title">Diagnostic système</div><div class="diag"><div>Pompe<span>${this._state(this.config.pump_entity)?.state || "non configurée"}</span></div><div>Météo<span>${this._state(this.config.weather_entity)?.state || "non configurée"}</span></div><div>Filtration auto<span>${autoSt}</span></div></div>${this._smartFiltrationInfo()}<div class="section-title">Actions Pool Pilot</div><div class="diag"><button class="expert-maintenance ${this._maintenanceActive() ? "active" : ""}" data-maintenance-toggle><span><ha-icon icon="mdi:tools"></ha-icon>Mode Maintenance</span><b>${this._maintenanceActive() ? "Activé" : "Désactivé"}</b></button></div>${this._chlorineLsiPanel()}</div></div>`;
  }
  ...
  _maintenanceEntity() {
    if (this.config?.maintenance_entity && this._state(this.config.maintenance_entity)) return this.config.maintenance_entity;
    const preferred = ["switch.piscine_mode_maintenance", "switch.piscine_maintenance_mode"];
    const found = preferred.find((entity) => this._state(entity)) || Object.keys(this._hass?.states || {}).find((entity) => entity.startsWith("switch.") && (entity.endsWith("_mode_maintenance") || entity.endsWith("_maintenance_mode")));
    return found || this.config?.maintenance_entity || preferred[0];
  }
  ...
  _recommendationAsAlert(rec) {
    if (!rec) return null;
    const rawQuantity = Number(rec.quantity);
    const quantity = Number.isFinite(rawQuantity)
      ? rawQuantity.toLocaleString("fr-FR", { maximumFractionDigits: 2 })
      : rec.quantity || "";
    const dosage = `Ajouter ${quantity} ${rec.unit || ""} de ${rec.product_name || "produit"}.`
      .replace(/\s+/g, " ")
      .trim();
    const steps = [dosage];
    if (rec.reason) steps.push(rec.reason);
    if (rec.aftercare) steps.push(rec.aftercare);
    return {
      id: "product_recommendation",
      title: "Correction recommandée",
      message: rec.reason || dosage,
      icon: "mdi:flask-plus",
      steps,
      recommendation: rec,
    };
  ...
  _primaryAlert() {
    ...
    const recommendations = s?.attributes?.recommendations;
    if (Array.isArray(recommendations) && recommendations.length)
      return this._recommendationAsAlert(recommendations[0]);
    const actionEntity = this._entityByKey ? this._entityByKey("action_summary") : null;
    const actionRecommendations = this._state(actionEntity)?.attributes?.recommendations;
    if (Array.isArray(actionRecommendations) && actionRecommendations.length)
      return this._recommendationAsAlert(actionRecommendations[0]);
    return null;
  }
  _alertSteps(alert) {
    if (alert?.steps && Array.isArray(alert.steps) && alert.steps.length) return alert.steps;
    const id = alert?.id || "";
    if (id === "storm") return ["Sortez immédiatement les baigneurs de l'eau.","Retirez les bouées et les objets pouvant s'envoler. Le capteur peut rester dans la piscine.","Vérifiez le niveau d'eau et abaissez la ligne d'eau d'environ 3 cm si nécessaire.","En cas de forte activité électrique, coupez l'alimentation de la pompe et des équipements électriques.",`Après l'orage, contrôlez le pH et la désinfection (${this._treatmentLabel()}) puis relancez la filtration si elle a été arrêtée.`];
    if (id === "green_algae_risk") return [this._treatmentBoostAdvice(),"Contrôlez le pH avant le traitement.","Brossez les parois et le fond du bassin.","Filtration continue pendant 24 à 48 h."];
    if (id === "water_cold") return ["Activez la pompe à chaleur.","Vérifiez la consigne de chauffage.","Laissez la filtration fonctionner normalement."];
    if (id === "water_hot") return ["Activez le refroidissement de la PAC si disponible.","Augmentez la filtration pendant les heures les plus chaudes.",`Contrôlez le pH et la désinfection (${this._treatmentLabel()}) quotidiennement.`];
    if (id === "stock_low") return ["Réapprovisionnez le produit indiqué dans le Pool House.","Mettez à jour le stock après achat."];
    if (id === "analyseur_eau_battery_low") return ["Remplacez ou rechargez la batterie du capteur.","Vérifiez que les mesures remontent de nouveau."];
...
;(() => {
  ...
  proto.setConfig = function poolPilotSetConfig(config) {
